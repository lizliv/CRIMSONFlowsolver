...
!                   there is a 2d arraydata to be sent. first index gives
!                   the global node number and the second index gives the
!                   value to be sent
...
contains
        ...
        subroutine write_restart_parallel()
                ...
                displacement = shoffset + sizeof(sh) + rank*sizeof(message)
                call MPI_File_set_view(fileno, displacement, MPI_BYTE, MPI_BYTE, 'native', MPI_INFO_NULL, ierr)
                call MPI_File_write(fileno, message, len(message), MPI_CHARACTER, &
                        wstatus, ierr)
                
                ! Alternative way to write the message
